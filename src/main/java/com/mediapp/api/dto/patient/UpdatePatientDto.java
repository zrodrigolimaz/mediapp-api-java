...
@Schema(description = "DTO para atualização de paciente (todos os campos são opcionais)")
public record UpdatePatientDto(

    @Schema(description = "Nome completo do paciente", example = "João Silva Santos", required = false)
    @Size(min = 1, message = "O nome completo deve ter pelo menos 1 caractere.")
    String fullName,

    @Schema(description = "CPF do paciente (formato XXX.XXX.XXX-XX ou 11 dígitos)", example = "123.456.789-00", required = false)
    @Pattern(regexp = "^\\d{3}\\.\\d{3}\\.\\d{3}-\\d{2}$|^\\d{11}$", message = "CPF deve estar no formato XXX.XXX.XXX-XX ou conter 11 dígitos.")
    String taxId,

    @Schema(description = "Data de nascimento do paciente", example = "1990-05-15", required = false)
    @Pattern(regexp = "^\\d{4}-\\d{2}-\\d{2}$", message = "A data de nascimento deve estar no formato YYYY-MM-DD.")
    String birthDate,

    @Schema(description = "Telefone de contato principal", example = "(11) 98765-4321", required = false)
    String contactPhone,
